...
std::vector<int> adjust(std::vector<Cell> &cells, std::vector<std::pair<int, int>> &block_list)
{
	...
	calc_blocks_placeability(cells, block_list, can_place_prefix);
	{
		std::vector<ReversedVector<bool>> can_place_suffix_rev;
		for(auto &vec : can_place_suffix) can_place_suffix_rev.push_back(ReversedVector(vec));
		calc_blocks_placeability(ReversedVector(cells), ReversedVector(block_list), can_place_suffix_rev);
	}

	calc_blocks_placeability(ReversedVector(cells), ReversedVector(block_list), can_place_suffix);
